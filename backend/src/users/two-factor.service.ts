...
export class TwoFactorService {
    ...
    public async generateTwoFactorSecret(userEmail: string) {
        ...
        const appName = this.configService.get('APP_NAME', 'Collector');
        ...
        const otpauthUrl = `otpauth://totp/${encodeURIComponent(label)}?secret=${secret}&issuer=${encodeURIComponent(appName)}&algorithm=SHA1&digits=6&period=30`;
    ...
    public verifyTwoFactorToken(token: string, secret: string): boolean {
        ...
        // verifySync returns boolean or VerifyResult?
        // Based on previous step, we assumed boolean or result object.
        const result = verifySync({
            token,
            secret,
        });

        // @ts-ignore: Handle potential type mismatch if VerifyResult is not boolean
